...
def create_lookup_tables(text):
    ...
    vocab_to_int = {key:value for value,key in enumerate(vocab)}
    int_to_vocab = {key:value for key,value in enumerate(vocab)}
...
def token_lookup():
    ...
    """
    # Create Punctuation to String dict
    return {'.':'||Period||',
            ',':'||Comma||',
            '"':'||Quotation_Mark||',
            ';':'||Semicolon||',
...
def get_inputs():
    ...
    """
    # Create input targets and leaning_rate
...
def get_init_cell(batch_size, rnn_size):
    ...
    """
    # Create init lstm cell and init the state
...
def get_embed(input_data, vocab_size, embed_dim):
    ...
    """
    # Create embedding perform embedding look up
...
def get_batches(int_text, batch_size, seq_length):
    ...
    data = np.zeros((num_batch, 2, batch_size, seq_length), dtype=int)
    count = num_batch * batch_size * seq_length
    for i in range(num_batch):
        for j in range(batch_size):
            for k in range(seq_length):
                index = i*seq_length + j*num_batch*seq_length + k
                data[i][0][j][k] = int_text[index]
                data[i][1][j][k] = int_text[(index + 1)%(count)]
            # print("i:{},j:{},data[0]:{},data[1]:{}".format(i, j, data[i][0][j], data[i][1][j]))
    return data
...
def pick_word(probabilities, int_to_vocab):
    ...
    # print("{} {}".format(len(probabilities), len(int_to_vocab)))
